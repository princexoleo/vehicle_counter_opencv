...
"""
# Initialize the important libraries
import numpy as np
import cv2 
import pandas as pd 
...
# convert the width and height to integer
width = int(width)
height = int(height)
print("[VideoProperties] frame_count:{}, fps:{:.2f}, width:{}, height:{} ".format(
    frames_count, fps, width, height
))

# Create a pandas dataframe with the number of rows the lenght as frame_count
...
ret, frame = cap.read() # import images
ratio = 0.5 # for resize the images
image = cv2.resize(frame, (0,0), None, ratio, ratio) # resize the image
width2, height2, channels = image.shape
video = cv2.VideoWriter("traffic_counter.mp4", cv2.VideoWriter_fourcc("M", "J","P","G"),
    fps, (height2, width2), 1
)

while True:
    ret, frame = cap.read() # read the images
    if ret: #if there is a frame containue with code
        image = cv2.resize(frame, (0, 0), None, ratio, ratio)  # resize image
        gray  = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) # convert the image into Gray
        fgmask = fgbg.apply(gray) # use the background subtractor

        # applies diffent theshold to fgmask to try isolate cars
        # just have to keep playing around with settings until cars are easily identifiable
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5,5)) # kernet to apply to the morphology
        closing = cv2.morphologyEx(fgmask, cv2.MORPH_CLOSE, kernel)
        opening = cv2.morphologyEx(closing, cv2.MORPH_OPEN, kernel)
        dilation = cv2.dilate(opening, kernel)
        retvalbin, bins = cv2.threshold(dilation, 220,255, cv2.THRESH_BINARY) # remove the shadows

        # creates contours
        contours, hierarchy = cv2.findContours(bins, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        # use conves hull to create polygon contours
        hull = [cv2.convexHull(c) for c in contours]
        # draw contours
        cv2.drawContours(image, hull, -1, (0,255,255),3)
        # line created to stop counting contours, need as caars in distance become one big contour
        linepos = 225
        ...
        minarea = 300
        # max area for contours, can be quitte large for buses
        maxarea = 50000

        # vectors for the x and y location of contour centroids in current frame
